...
ModulesToPath <- function(path) {
  df <- as.data.frame(do.call(rbind,strsplit(readLines('http://api.kegg.net/link/pathway/module'),'\t',fixed=T)),stringsAsFactors = F)
  df$V2 <- sub("map","ko",df$V2)
  df <- df[df$V2 %in% path$path_id,]
  df <- merge(out,df,by.x="path_id",by.y="V2",all=T)
  #add module names
  df2 <- as.data.frame(do.call(rbind,strsplit(readLines('http://api.kegg.net/list/module'),'\t',fixed=T)),stringsAsFactors = F)
