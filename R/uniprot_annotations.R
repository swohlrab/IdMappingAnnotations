...
uniprot_annotations <- function(ids,database) {
  ifelse(length(ids) > 1000, ids <- split(ids, ceiling(seq_along(ids)/1000)),ids <- list(ids))
  parseIDs <- function(x) {
  ...
  query <- dat1$To
  ifelse(length(query)>1000,query<-split(query, ceiling(seq_along(query)/1000)),query<-list(query))
  to <- c('ID',database)
